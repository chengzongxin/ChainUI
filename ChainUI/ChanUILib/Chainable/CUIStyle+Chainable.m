//
//  CUIStyle+Chainable.m
//  ChainUI
//
//  Created by Joe.cheng on 2021/3/1.
//

#import "CUIStyle+Chainable.h"
#import "CUIUtils.h"
...
- (CUIChainableCUIStyle##type1##Block)name {\
    CUI_##type2##_BLOCK([self set##type1##Value:value forKey:@#name]);\
}

#define SYNTHSIZE_STRUCT_METHOD(type1, type2, name) \
- (CUIChainableCUIStyle##type1##Block)name {\
CUI_##type2##_BLOCK([self set##type1##Value:value.value forKey:@#name]);\
}

#define SYNTHSIE_FLOAT_LIST_METHOD(name) \
- (CUIChainableCUIStyleFloatListBlock)name {\
    CUI_FLOAT_LIST_BLOCK([self setFloatListValue:value forKey:@#name]);\
}
...
@implementation CUIStyle (Chainable)

- (CUIChainableCUIStyleObjectBlock)styles {
    CUI_OBJECT_BLOCK([CUIUtils applyStyleObject:value toItem:self];);
}

- (CUIChainableCUIStyleFloatObjectListBlock)border {
...
- (CUIChainableCUIStyleObjectBlock)fnt {
    CUI_OBJECT_BLOCK([self setObjectValue:Fnt(value) forKey:@"fnt"]);
}

- (CUIChainableCUIStyleObjectBlock)detailFnt {
    CUI_OBJECT_BLOCK([self setObjectValue:Fnt(value) forKey:@"detailFnt"]);
}

- (CUIChainableCUIStyleInsetsBlock)insets {
    CUI_INSETS_BLOCK([self setObjectValue:[NSValue valueWithUIEdgeInsets:value] forKey:@"insets"]);
}

- (CUIChainableCUIStyleInsetsBlock)thumbInsets {
    CUI_INSETS_BLOCK([self setObjectValue:[NSValue valueWithUIEdgeInsets:value] forKey:@"thumbInsets"]);
}
