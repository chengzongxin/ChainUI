...
id CUIConvertValueToString(const char *type, ...) {
    ...
    if (CUI_CHECK_IS_INT(type[0])) {
        NSInteger n = va_arg(argList, NSInteger);
        result = [@(n) description];
    } else if (CUI_CHECK_IS_FLOAT(type[0])) {
...
NSString *CUIStringRepresentationOfValue(const char *type, const void *value) {
    ...
    #define CHECK_IS_TYPE_OF(t1, t2)    if (CUI_IS_TYPE_OF(t1)) return NSStringFrom##t2(VALUE_OF_TYPE(t1))
