...
@interface CUITransformLayer : CATransformLayer

@end

@implementation CUITransformLayer
...
- (void)setAlignment:(CUIStackAlignment)alignment {
...
- (void)addAndActivateConstraintsForAll {
    ...
    for (int i = 0; i < self.arrangedSubviews.count; ++i) {
        ...
        if ([item isKindOfClass:[CUIStackSpring class]]) {
            CUIStackSpring *spring = (CUIStackSpring *)item;
...
- (void)addAndActivateConstraintsForViewAtIndex:(NSInteger)index {
    ...
    UIView *item = ITEM_AT_INDEX(index);
    if ([item isKindOfClass:CUIStackSpring.class]) {
        ((CUIStackSpring *)item).axis = self.axis;
...
- (NSLayoutConstraint *)addAlignmentConstraintForIndex:(NSInteger)index {
    ...
    if (self.axis == UILayoutConstraintAxisVertical) {
        if (self.alignment == CUIStackAlignmentLeading) att = NSLayoutAttributeLeading;
        if (self.alignment == CUIStackAlignmentTrailing) att = NSLayoutAttributeTrailing;
        if (self.alignment == CUIStackAlignmentCenter) att = NSLayoutAttributeCenterX;
    } else {
        if (self.alignment == CUIStackAlignmentTop) att = NSLayoutAttributeTop;
        if (self.alignment == CUIStackAlignmentBotttom) att = NSLayoutAttributeBottom;
        if (self.alignment == CUIStackAlignmentCenter) att = NSLayoutAttributeCenterY;
        if (self.alignment == CUIStackAlignmentFirstBaseline) att = NSLayoutAttributeFirstBaseline;
        if (self.alignment == CUIStackAlignmentBaseline) att = NSLayoutAttributeLastBaseline;
    }
...
- (void)addAndActivateSpringConstraintsForSpringAtIndex:(NSInteger)index {
    for (int i = 0; i < self.arrangedSubviews.count; ++i) {
        ...
        if ([sv isKindOfClass:CUIStackSpring.class] && i != index) {
...
+ (instancetype)verticalStackWithItems:(NSArray *)items {
    CUIStack *stack = [CUIStack new];
    stack.axis = UILayoutConstraintAxisVertical;
    stack.alignment = CUIStackAlignmentLeading;
...
+ (instancetype)horizontalStackWithItems:(NSArray *)items {
    ...
    stack.alignment = CUIStackAlignmentCenter;
...
+ (Class)layerClass {
    return [CUITransformLayer class];
}
