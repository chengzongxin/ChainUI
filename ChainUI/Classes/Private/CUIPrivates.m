...
@implementation NSObject (CUIPrivate)
...
- (void)cui_setParagraphStyleValue:(id)value forKey:(NSString *)key range:(NSRange)range {
    ...
    if ([key isEqualToString:@"lineSpacing"]) {
        [self addAttribute:CUIFixLineSpacingAttributeName value:CUIFixLineSpacingAttributeValue range:range];
    }
}
...
- (instancetype)cui_updateFrame:(CUIRect)rect {
    ...
    if (frame.origin.x != CUINull) {
        newFrame.origin.x = frame.origin.x;
    }
    if (frame.origin.y != CUINull) {
        newFrame.origin.y = frame.origin.y;
    }
    if (frame.size.width != CUINull) {
        newFrame.size.width = frame.size.height;
    }
    if (frame.size.height != CUINull) {
...
@implementation UIImageView (CUIPriavte)
